...
class ComboTest extends IntegrationTestBase {
    ...
    class GetComboDetailsById {
        ...
        @Test
        @DisplayName("GET /api/combos/{id}/details should return 403 Forbidden when manager tries to access combo from different restaurant (cross-tenant)")
        void shouldReturnForbiddenForCrossTenantAccessComboById() {
            given()
    ...
    class CreateComboTest {
        ...
        @Test
        @DisplayName("POST /api/combos/ should return 403 Forbidden when creating combo with foreign product ID and NOT create in DB")
        void shouldReturnForbiddenWhenUsingForeignProduct() {
            comboBuilder
                    .withName("Cross Tenant Combo")
                    .withPrice(new BigDecimal("50.00"))
                    .clearComponents()
                    .withComponent(1L, 1);

            long countBefore = comboRepository.count();
            ...
            long countAfter = comboRepository.count();
            Assertions.assertEquals(countBefore, countAfter, "No new combo should be created in DB after cross-tenant injection attempt");
        }

        @Test
        @DisplayName("POST /api/combos/ should return 403 Forbidden and NOT create combo in DB when user is EMPLOYEE")
        void shouldReturnForbiddenAndNotCreateComboWhenEmployee() {
            comboBuilder
                    .withName("Employee Combo Attempt")
                    .withPrice(new BigDecimal("40.00"))
                    .clearComponents()
                    .withComponent(1L, 1);
            ...
            // Verification of the absence of side effects in DB
            long countAfter = comboRepository.count();
            Assertions.assertEquals(countBefore, countAfter, "No new combo should be created in DB after EMPLOYEE attempt");
        }

        @Test
        @DisplayName("POST /api/combos/ should return 400 Bad Request when component quantity is zero")
        void shouldReturnBadRequestWhenComponentQuantityIsZero() {
            comboBuilder
                    .withName("Combo with zero quantity")
                    .withPrice(new BigDecimal("50.00"))
                    .clearComponents()
                    .withComponent(1L, 0);
        ...
        @Test
        @DisplayName("POST /api/combos/ should return 400 Bad Request when component quantity is negative")
        void shouldReturnBadRequestWhenComponentQuantityIsNegative() {
            comboBuilder
                    .withName("Combo with negative quantity")
                    .withPrice(new BigDecimal("50.00"))
                    .clearComponents()
                    .withComponent(1L, -1);
    ...
    class PatchComboTest {
        ...
        @Test
        @DisplayName("PATCH /api/combos/{id} should return 403 and NOT change DB when manager from different restaurant tries to access")
        void shouldReturnForbiddenForCrossTenantAccess() {
            ...
            // Verification of the absence of side effects in DB
            Optional<Combo> entity = comboRepository.findById(1L);
            Assertions.assertTrue(entity.isPresent());
            Assertions.assertNotEquals(new BigDecimal("10.00"), entity.get().getPrice());
        }
        ...
        @Test
        @DisplayName("PATCH /api/combos/{id} should return 400 Bad Request when component quantity is zero")
        void shouldReturnBadRequestWhenComponentQuantityIsZero() {
            ComboTestBuilder patchBuilder = new ComboTestBuilder()
                    .withName("Patched Combo")
                    .withPrice(new BigDecimal("50.00"))
                    .clearComponents()
                    .withComponent(1L, 0);
        ...
        @Test
        @DisplayName("PATCH /api/combos/{id} should return 400 Bad Request when component quantity is negative")
        void shouldReturnBadRequestWhenComponentQuantityIsNegative() {
            ComboTestBuilder patchBuilder = new ComboTestBuilder()
                    .withName("Patched Combo")
                    .withPrice(new BigDecimal("50.00"))
        ...
        @Test
        @DisplayName("PATCH /api/combos/{id} should return 403 Forbidden when patching with foreign product ID (cross-tenant component injection)")
        void shouldReturnForbiddenForCrossTenantComponentInjection() {
            // Manager of restaurant 2 attempts to patch combo from restaurant 1 with product IDs from restaurant 1
            ComboTestBuilder patchBuilder = new ComboTestBuilder()
                    .withName("Cross Tenant Injected Combo")
                    .withPrice(new BigDecimal("50.00"))
                    .clearComponents()
                    .withComponent(1L, 1);
            ...
            Optional<Combo> entity = comboRepository.findById(1L);
            Assertions.assertTrue(entity.isPresent());
            Assertions.assertNotEquals("Cross Tenant Injected Combo", entity.get().getName(),
                    "Combo name should not be changed after cross-tenant component injection attempt");
        }
    }
    ...
    class DeleteComboTest {
        ...
        @Test
        @DisplayName("DELETE /api/combos/{id} should return 403 and NOT delete from DB when manager tries to delete combo from different restaurant")
        void shouldReturnForbiddenForCrossTenantAccessDelete() { // Zmiana nazwy metody, aby uniknąć duplikacji
            given()
                    .spec(authSpecManagerRestaurantWithoutProducts())
                    .when()
                    .delete(COMBO_BASE_URL + "/{id}", 1L)
            ...
            // Verification of the absence of side effects in DB
            Optional<Combo> entity = comboRepository.findById(1L);
            Assertions.assertTrue(entity.isPresent(), "Combo should not be deleted");
        }
