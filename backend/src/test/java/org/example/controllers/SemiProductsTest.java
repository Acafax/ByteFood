...
public class SemiProductsTest extends IntegrationTestBase {
    ...
    class CreateSemiProductTest {
        ...
        @Test
        @DisplayName("POST /api/semi-products - Should return 403 Forbidden and NOT create semi-product in DB when user is EMPLOYEE")
        void shouldReturnForbiddenAndNotCreateSemiProductWhenEmployee() {
            Map<String, Object> semiProduct = semiProductTestBuilder
                    .withName("Employee Semi Product Attempt")
                    .buildMap();

            long countBefore = semiProductRepository.count();
            ...
            long countAfter = semiProductRepository.count();
            Assertions.assertEquals(countBefore, countAfter, "No new semi-product should be created in DB after EMPLOYEE attempt");
        }
    }
    ...
    class PatchSemiProductTest {
        ...
        @Test
        @DisplayName("PATCH /api/semi-products/{id} - Should return 400 Bad Request when nutritional values are negative")
        void shouldReturnBadRequestWhenPatchNutritionalValuesAreNegative() {
            Map<String, Object> patchData = Map.of("carbohydrate", -10.0);
            ...
            Map<String, Object> patchDataFat = Map.of("fat", -5.0);
            ...
            Map<String, Object> patchDataProtein = Map.of("protein", -1.0);

            given()
                .spec(authSpec(UserRole.MANAGER))
                .body(patchDataProtein)
            .when()
                .patch(SEMI_PRODUCTS_URL + "/{id}", 1L)
            .then()
                .statusCode(400);
        ...
        @Test
        @DisplayName("PATCH /api/semi-products/{id} - Should return 403 and NOT change DB when manager from different restaurant tries to patch")
        void shouldReturnForbiddenWhenManagerFromDifferentRestaurant() {
            Map<String, Object> patchData = Map.of("name", "NEW SEMI NAME");
            ...
            Optional<SemiProduct> entity = semiProductRepository.findById(1L);
            Assertions.assertTrue(entity.isPresent());
            Assertions.assertNotEquals("NEW SEMI NAME", entity.get().getName());
        }
    ...
    class DeleteSemiProductTest {
        ...
        @Test
        @DisplayName("DELETE /api/semi-products/{id} - Should return 403 and NOT delete from DB when manager from different restaurant tries to delete")
        void shouldReturnForbiddenWhenManagerFromDifferentRestaurant() {
            given()
                    .spec(authSpecManagerRestaurantWithoutProducts())
                    .when()
            ...
            // Verification of the absence of side effects in the DB
            Optional<SemiProduct> entity = semiProductRepository.findById(1L);
            Assertions.assertTrue(entity.isPresent(), "SemiProduct should not be deleted");
        }
        @Test
