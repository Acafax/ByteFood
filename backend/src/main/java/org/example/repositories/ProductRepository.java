...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query("SELECT p.id as id, p.name as name, p.price as price, p.category as category, p.restaurantId as restaurantId " +
            "FROM Product p WHERE p.id IN :ids ")
    Set<ProductProjection> findProductsByIds(@Param("ids") Set<Long> ids);
    ...
    @Query("SELECT p FROM Product p " +
            ...
            "LEFT JOIN FETCH psp.semiProduct " +
            "WHERE p.id IN :ids " )
    Set<Product> findProductsWithProductSemiProductIdsByIds(@Param("ids") Set<Long> ids);
