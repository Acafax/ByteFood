...
function Magazyn() {
  ...
  const inventory = safeStock.map((item) => {
    const semiProduct = item.semiProductDTO || {};
    const unitLabel = semiProduct.unit ? semiProduct.unit.toLowerCase() : '';
    return {
      id: item.id,
      name: semiProduct.name || '—',
      quantity: item.quantity ?? 0,
      minStock: semiProduct.minimalStockQuantity ?? 0,
      unit: unitLabel,
      status: deriveStatus(item.quantity ?? 0, semiProduct.minimalStockQuantity ?? 0),
    };
  });
